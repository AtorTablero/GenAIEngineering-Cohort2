...
(function makeStars(){
    ...
    for (let i = 0; i < 120; i++) {
        ...
        // twinkle animation timing per-star
        const dur = (1.6 + Math.random() * 2.4).toFixed(2) + 's';
        star.style.animation = `twinkle ${dur} infinite`;
...
const openEnrollBtns = [document.getElementById('openEnroll'), document.getElementById('heroEnroll')];
const closeModalBtn = document.getElementById('closeModal');
const cancelBtn = document.getElementById('cancelEnroll');
const submitBtn = document.getElementById('submitEnroll');

function openModal(){ enrollModal.classList.add('open'); enrollModal.setAttribute('aria-hidden','false'); document.getElementById('nameInput').focus(); }
function closeModal(){ enrollModal.classList.remove('open'); enrollModal.setAttribute('aria-hidden','true'); }

openEnrollBtns.forEach(b => b && b.addEventListener('click', openModal));
...
submitBtn.addEventListener('click', ()=>{
    ...
    if (!name || !email) { msg.style.display='block'; msg.textContent='Please fill both name and email.'; return; }
    if (!isEmailValid(email)) { msg.style.display='block'; msg.textContent='Please enter a valid email address.'; return; }
    // Save to localStorage as simple mock
    const list = JSON.parse(localStorage.getItem('genai_enroll') || '[]');
    list.push({name, email, ts: new Date().toISOString()});
    localStorage.setItem('genai_enroll', JSON.stringify(list));
    msg.style.display='block'; msg.textContent = `You're on the list — we'll email ${email} with next steps!`;
    submitBtn.textContent='Saved ✔'; submitBtn.disabled = true;
    setTimeout(()=>{ closeModal(); submitBtn.textContent='Save my spot'; submitBtn.disabled=false; msg.style.display='none'; document.getElementById('nameInput').value=''; document.getElementById('emailInput').value=''; }, 1600);
});

// Testimonials carousel
(function rotateTestimonials(){
    const elems = Array.from(document.querySelectorAll('.testimonial'));
    let i = 0; if (!elems.length) return;
    function show(idx){ elems.forEach((el,j)=> el.style.opacity = j===idx? '1' : '0.35'); }
    show(i);
    setInterval(()=>{ i = (i+1)%elems.length; show(i); }, 3500);
})();

// Accordion
document.querySelectorAll('.accordion').forEach(acc => {
    const btn = acc.querySelector('button'); const panel = acc.querySelector('.panel');
...
if (previewBtn) previewBtn.addEventListener('click', ()=> alert('Preview: Course studio demo (placeholder)'));
const learnMore = document.getElementById('learnMore'); if (learnMore) learnMore.addEventListener('click', ()=> document.getElementById('curriculum').scrollIntoView({behavior:'smooth'}));
...
(function neuralBackground(){
    ...
    const { sparkHueBase } = getPaletteColors();
    const hue = sparkHueBase + Math.random()*40 - 10;
    ...
    planetGrad.addColorStop(1, `rgba(0,0,0,0.0)`);
        ...
        edges.forEach(e=>{
            ...
            grad.addColorStop(0, `rgba(${nodeA},${0.08*intensity})`);
            grad.addColorStop(1, `rgba(${nodeB},${0.06*intensity})`);
    ...
    // reuse nodeA/nodeB extracted above
    nodes.forEach((n,idx)=>{
            n.pulse += dt*0.002;
            // apply hover pulse
            ...
            gr.addColorStop(0, `rgba(${nodeA},${Math.min(1,0.92*pulse*intensity)})`);
            gr.addColorStop(1, `rgba(${nodeB},${0.02*pulse})`);
            ctx.fillStyle = gr;
    ...
    coreGlow.addColorStop(0, `rgba(${nodeB},0.28)`);
    ...
    // inner pattern
    ctx.fillStyle = `rgba(${nodeA},0.06)`; ctx.beginPath(); ctx.arc(cpuX, cpuY, cpuR*0.6,0,Math.PI*2); ctx.fill();
    // radial pins
    ctx.fillStyle = `rgba(${nodeB},0.09)`;
    ...
    const { nodeA: nodeAStr } = getPaletteColors();
    const ncol = nodeAStr.split(',').map(Number);
        ...
        for (let i = 0; i < sparks.length; i++) {
            const { edge, t, color } = sparks[i];
            const p = getEdgePoint(edge, t);
            sparkPos[i*2] = p.x;
            sparkPos[i*2+1] = p.y;
            sparkSize[i] = 6 + 8 * Math.sin(t * Math.PI) * intensity;
            const m = color.match(/hsl\((\d+),/);
            const hue = m ? parseInt(m[1], 10) : 200;
            const rgb = hslToRgb(hue, 0.8, 0.6);
            sparkCol[i*3] = rgb[0];
            sparkCol[i*3+1] = rgb[1];
            sparkCol[i*3+2] = rgb[2];
        }
    ...
    const { nodeA: nodeALineStr } = getPaletteColors(); const lc = nodeALineStr.split(',').map(Number);
